...
As an illustration, let's focus on one *Paisley dataset*, one of the historical sources we will be exploring here. The raw data is stored as an excel spreadsheet named "paisley-data.xlsx" in the folder "data". The command `readxl()` imports this excel file into the R environment. Although `readxl` is contained in the tidyverse package, you still need to open it explicitly using `library()`.
