...
```{r}
data_token |> select(President, Year, words)
```
...
data_token |> select(Year, President, words)
```
...
Likewise, instead of splitting the corpus into 1-gram tokens, tokenising may involve creating multiple-word tokens: 2-grams, 3-grams, etc. As above, the code below uses `unnest_tokens()` again but it now indicates that we want 5-gram tokens (`n = 5`). The output shows how the corpus has been split into 5-token grams.

```{r}
data_token2 <- speeches |>
  unnest_tokens(window, Text, token = "ngrams", n = 5)
data_token2 |> select(President, Year, window)
```

We can now focus only on those ngrams (windows) mentioning particular terms. Imagine, for instance, that we are not only interested in how many times the word "women" (or "woman") is mentioned, but also in which context. The code below implements such analysis (we could have expanded the "window" to more terms if we wished). 
...
As well as to learn more about the general context in which these terms appear, this type of multiple-word tokens are very useful to identify the most common (or rare) terms that are mentioned accompanying particular words. The code below builds on the previous object, puts all the accommpanying words in the same column (but keeping the information about their position) and removes stop words.
...
Identifying which terms are most commonly mentioned around women in the straightforward. Here, we take into account all of them, regardless where the were mentioned but we could use the info on the column *order* to qualify this analysis.
...
Many of the words mentioned around women refer to their role as mothers "children", "pregnant", "infant", etc. (although this also arises from referring to "women and children" as a group). As well as to the nation itself ("america", "american"), we can also identify words related to employment and to minorities: "chinese", "indian" and the term "minorities" itself. 

An additional exercise could involve tracing the temporal evolution in which these categories emerge. Here we just simply identify the years in which the term "pregnant" shows up. This seem to be an issue that pertains especially the 1980s and 1990s and this would push you to read more about those speeches and what was the particular context behind.
...
We are going to stop here. The analysis above is very rudimentary and suffers from many issues but we don't have time for more. There is in any case so much more that we could do. I should also stress that these methods are a complement (not a substitute) to close reading, source criticism and deep contextual knowledge.

----

I hope these two sessions have given you a sense of how computational methods allow extracting information from historical sources in a powerful way, regardless whether the information is qualitative, numerical or purely textual. For those who want to know more, see you in [HIST2025 Computational History](https://www.ntnu.edu/studies/courses/HIST2025#tab=omEmnet).
