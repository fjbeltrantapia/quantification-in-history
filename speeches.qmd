...
As well as to learn more about the general context in which these terms appear, this type of multiple-word tokens are very useful to identify the terms that are mentioned accompanying particular words. Imagine, for instance, that we are not only interested in how many times the word "women" is mentioned, but also in which context. The code below implements such analysis (we could have expanded the "window" to more terms if we wished).
...
women <- data_token2 |> 
  ...
  rename(words = value) |>
  anti_join(stop_words, by = c("words" = "word")) 
women |> select(President, Year, order, words)
```
...
Many of the words mentioned around women refer to their role as mothers: "children", "pregnant", "infant", etc. We can also identify words related to employment and to minorities: "chinese", "indian" and the term "minorities" itself. 
