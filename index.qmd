...
**3. Download the following materials**. Store these files in separate folders (named `data` and `scripts`, respectively) within the folder `quants`:
  ...
  * The [datasets]{.underline} we will be working with: [here](data/paisley-data.xlsx) and [here](data/state-of-the-union-texts.csv). 
    
  * The [R scripts]{.underline} containing the code to extract (some) information from them: [here](paisley.R) and [here](speeches.R).
